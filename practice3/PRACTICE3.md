...
Для отправки приглашений участникам встречи из пода `booking-service` в кластере задайте переменные SMTP (например, расширив манифест деплоймента или вынеся их в отдельный Secret); локально они задаются в `practice2/.env` (см. `.env.example`).
