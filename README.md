...
5. Проверьте в браузере или через `curl`:
   ...
   **Участники встречи:** организатор (владелец брони) может в интерфейсе найти зарегистрированного пользователя по фрагменту email (`GET /api/users/search`) и добавить его к выбранной брони; на адрес участника отправляется письмо-приглашение, если в окружении `booking-service` заданы переменные SMTP (`SMTP_HOST`, `SMTP_PORT`, и т.д., см. `practice2/.env.example`). Без SMTP участник сохраняется в системе, письмо пропускается (в лог пишется информационное сообщение).
