...
Скрипт отправляет 150 запросов к API и позволяет наблюдать рост RPS и latency на дашборде. После появления маршрутов поиска пользователей и участников брони дополнительная нагрузка на gateway отражается в тех же метриках по полю `path`.
